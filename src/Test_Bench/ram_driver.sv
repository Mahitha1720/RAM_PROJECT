...
class ram_driver;
  ...
  ignore_bins wr_rd_bin = binsof(WRITE) intersect {1}
                        && binsof(READ) intersect {1};

ignore_bins wr_rd_bin2 = binsof(WRITE) intersect {0}
    ...
    for (int i = 0; i < `num_transactions; i++) begin
      ...
      if (vif.drv_cb.reset == 0)
        repeat(1) @(vif.drv_cb) begin
          ...
          repeat(1) @(vif.drv_cb);
 $display(
            "DRIVER DRIVING DATA TO THE INTERFACE data_in=%0h, write_enb=%0d, read_enb=%0d, address=%0h",
            vif.drv_cb.data_in,
        ...
        repeat(1) @(vif.drv_cb) begin
          ...
          $display(
            "DRIVER WRITE OPERATION DRIVING DATA TO THE INTERFACE data_in=%0h, write_enb=%0d, read_enb=%0d, address=%0h",
            vif.drv_cb.data_in,
            vif.drv_cb.write_enb,
            vif.drv_cb.read_enb,
            vif.drv_cb.address,
